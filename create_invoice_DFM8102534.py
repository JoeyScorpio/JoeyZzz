#!/usr/bin/env python3
"""Generate Excel for invoice DFM8102534."""
...
for i, text in enumerate(bill_to):
    r = 7 + i
...
items = [
    ...
    (4, 'LB02P', 'FG 龙虾球 LOBSTER BALL（FG）', 3.00, 'PKT', 3.50, 10.50),
    (5, 'NTU01', '豆福手工卤水大豆腐 DOLFORD NORTH TOFU', 8.00, 'PCS', 1.20, 9.60),
    (6, 'SDO01', '烟熏鸭胸原味 Smoked Duck Original 5PKT/KG', 1.14, 'KGS', 9.50, 10.83),
    (7, 'ZSB0080', '金包银 MUSHROOM FISHN SOY 300G*30PKT/CTN', 2.00, 'PKT', 2.65, 5.30),
    (8, 'DB01P', '豆卜 DARK BROWN BEAN CURD 10PCS X 200G', 10.00, 'PKT', 1.40, 14.00),
    (9, 'SAT01', '豆福五香豆干（散装）1KG DOLFORD SPICED', 1.00, 'KGS', 3.60, 3.60),
    (10, 'VYC03', '豆福素鸡（散装） DOLFORD VEGETARIAN', 1.00, 'KGS', 3.60, 3.60),
    (11, 'ZSB0082', '蟹棒 MUSHROOM IMI.CRAB STICK', 15.00, 'PKT', 1.70, 25.50),
]
...
# Empty rows to fill table area
last_data_row = 17 + len(items) - 1  # 27
...
sub_total = sum(item[6] for item in items)
...
ws[f'F{summary_row}'].alignment = right_align
ws[f'G{summary_row}'] = round(sub_total + gst, 2)
...
output_path = '/home/user/JoeyZzz/invoice_DFM8102534.xlsx'
wb.save(output_path)
